...
public class QueryConstants {
	 ...
	 public static final String PROJECTHIERARCHY_SELECT = "SELECT PI.PROJECTID AS PROJECTID,PI.PROJECTNAME AS PROJECTNAME,PI.MCAFLAG AS MCAFLAG,R.REQUESTERID AS REQUESTERID,  R.REQUESTERNAME AS REQUESTERNAME,  P.PROCESSORID AS PROCESSORID,  P.PROCESSORNAME AS PROCESSORNAME, PM.STATUS AS PROJSTATUS,PM.ID AS PROJMAPID FROM PROJECTMAPPING PM  JOIN  REQUESTERS R ON R.REQUESTERID = PM.REQUESTERID JOIN  PROCESSORS P ON P.PROCESSORID = PM.PROCESSORID JOIN PROJECTINFO PI ON PI.PROJECTID = PM.PROJECTID";
	 public static final String EMPPROJHIST_SELECT = "SELECT  EPH.ID AS ID, EPH.ONBOARDREQUESTER AS ONBOARDREQUESTER,EPH.OFFBOARDREQUESTER AS OFFBOARDREQUESTER,EPH.OFFBOARDPROCESSOR AS OFFBOARDPROCESSOR, EPH.STARTDATE AS STARTDATE,  EPH.NATIONWIDEID AS NATIONWIDEID,  EPH.NATIONWIDEIDCREATEDDATE AS NATIONWIDEIDCREATEDDATE,  EPH.FGONBOARDINGDATE AS FGONBOARDINGDATE,  EPH.BAND AS BAND,  EPH.MOVEMENTID AS MOVEMENTID,  EPH.WORKFORCEID AS WORKFORCEID,  EPH.SCRUMMASTER AS SCRUMMASTER,  EPH.APM AS APM,  EPH.PPLMANAGER AS PPLMANAGER,  EPH.EXPERIENCE AS EXPERIENCE,  EPH.COMMENTS AS COMMENTS,  EPH.SKILLSET AS SKILLSET,  EPH.SKILLSUMMARY AS SKILLSUMMARY,  EPH.RELEASEDATE AS RELEASEDATE,  EPH.APPROVALSTATUS AS APPROVALSTATUSID,  APS.STATUS AS APPROVALSTATUS,  EPH.RELEASESTATUS AS RELEASESTATUSID,  RS.STATUS AS RELEASESTATUS,  EPH.REASONFOROFFBOARDING AS REASONFOROFFBOARDING,  EPH.ATTACHMENTID AS ATTACHMENTID,  EPH.EMPLOYEEID AS EMPLOYEEID,  EM.NAME AS NAME,  EM.FIRSTNAME AS FIRSTNAME,  EM.LASTNAME AS LASTNAME,  EM.DOB AS DOB,  EM.PASSPORTNUMBER AS PASSPORTNUMBER,  EM.EMAIL AS EMAIL,  EPH.COUNTRY AS COUNTRYID,  C.COUNTRY AS COUNTRYNAME,  L.LOCATION AS LOCATION,  EPH.ROLE AS ROLEID,  RO.ROLE AS ROLE,  RT.RATE AS RATE, EPH.ISRATEOVERRIDE AS ISRATEOVERRIDE,EPH.OVERRIDDENRATE AS OVERRIDDENRATE, EPH.TEAM AS TEAMID,  T.TEAMNAME AS TEAMNAME,EPH.BSA AS BSAID,  B.NAME AS BSANAME, PM.ID AS PROJECTMAPPINGID, PM.PROJECTID AS PROJECTID, PF.MCAFLAG AS MCAFLAG, PF.PROJECTNAME AS PROJECTNAME,  RQ.REQUESTERID AS REQUESTERID,  RQ.REQUESTERNAME AS REQUESTERNAME,  PR.PROCESSORID AS PROCESSORID,  PR.PROCESSORNAME AS PROCESSORNAME FROM  EMPLOYEEPROJECTHISTORY EPH JOIN  EMPLOYEEMASTER EM ON EM.ID = EPH.EMPLOYEEID JOIN  PROJECTMAPPING PM ON PM.ID = EPH.PROJECT JOIN  TEAMS T ON T.ID = EPH.TEAM JOIN  BSA B ON B.ID = EPH.BSA JOIN  PROJECTINFO PF ON PF.PROJECTID = PM.PROJECTID JOIN  REQUESTERS RQ ON RQ.REQUESTERID = PM.REQUESTERID JOIN  PROCESSORS PR ON PR.PROCESSORID = PM.PROCESSORID JOIN  ROLEMAPPING RM ON RM.ID = EPH.ROLE JOIN  ROLES RO ON RO.ID = RM.ROLEID JOIN  RATES RT ON RT.ID = RM.RATEID JOIN  COUNTRYMAPPING CM ON CM.ID = EPH.COUNTRY JOIN  COUNTRIES C ON C.ID = CM.COUNTRYID JOIN  LOCATIONS L ON L.ID = CM.LOCATIONID JOIN  APPROVALSTATUS APS ON APS.ID = EPH.APPROVALSTATUS JOIN  RELEASESTATUS RS ON RS.ID = EPH.RELEASESTATUS";
	 ...
	 public static final String MAILATTACHMENT_SELECT = "SELECT ID,ONBOARDFILENAME,OFFBOARDFILENAME,ONBOARDFILEDATA,OFFBOARDFILEDATA FROM MAILATTACHMENTS ";
	 ...
	 public static final String USERS_SELECT = "SELECT U.USERNAME AS USERNAME, U.PASSWORD AS PASSWORD, U.ENABLED AS ENABLED FROM USERS U";
	 ...
	 public static final String ONBOARDPROCESS_UPDATE = "UPDATE EMPLOYEEPROJECTHISTORY SET NATIONWIDEID = ?, NATIONWIDEIDCREATEDDATE = ?, FGONBOARDINGDATE = ?, WORKFORCEID = ?, SCRUMMASTER = ?, APM = ?, PPLMANAGER = ?, BAND = ?, ATTACHMENTID = ?, COMMENTS = ?, APPROVALSTATUS = ? WHERE  ID = ? ";
	 public static final String PROCESS_OFFBOARD_UPDATE = "UPDATE EMPLOYEEPROJECTHISTORY SET RELEASESTATUS = ? WHERE  ID = ? ";
	 public static final String REQUEST_OFFBOARD_UPDATE = "UPDATE EMPLOYEEPROJECTHISTORY SET RELEASESTATUS = ?, RELEASEDATE = ?, REASONFOROFFBOARDING = ?, OFFBOARDREQUESTER = ?, OFFBOARDPROCESSOR = ? WHERE  ID = ? ";
	 public static final String RELEASE_ATTACHMENT_UPDATE = "UPDATE MAILATTACHMENTS SET OFFBOARDFILENAME = ? , OFFBOARDFILEDATA = ? WHERE ID = ? "; 
	 public static final String USERS_UPDATE = "UPDATE USERS SET PASSWORD = ?,ENABLED = 1 WHERE USERNAME = ? ";
